...
const MailLog = props => {
  ...
  const handleFolderUpdate = (id, folder) => {
    const arr = Array.isArray(id) ? [...id] : [id]
    dispatch(updateMailFolder({ emailIds: arr, folder }))
  }
  ...
  const renderMailLabels = arr => {
    return arr.map((label, index) => {
      return (
        <Box key={index} component='span' sx={{ mr: 3.5, color: `${labelColors[label.toLowerCase()]}.main` }}>
            ...
            {store && store.mails && store.mails.length ? (
              <List sx={{ p: 0 }}>
                {store.mails.map(mail => {
                  const mailReadToggleIcon = mail.is_read ? 'mdi:email-outline' : 'mdi:email-open-outline'
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <Tooltip placement='top' title={mail.is_read ? 'Unread Mail' : 'Read Mail'}>
                          <IconButton
                            onClick={e => {
                              e.stopPropagation()
                              handleReadMail([mail.id], !mail.is_read)
                            }}
                          >
